        ...
        <ul role="list" class="flex flex-1 flex-col gap-y-7">
            <li>
                <ul role="list" class="-mx-2 space-y-1">
                    ...
                    @if($user && $user->hasPermission('view_messages'))
                        <li>
                            <a href="{{ route('outgoingemails.index') }}" class="group flex gap-x-3 rounded-md p-2 text-sm font-semibold {{ request()->routeIs('outgoingemails.*') ? 'bg-gray-100 text-gray-900' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900' }}">
                                <!-- SVG Icon -->
                                <svg class="h-6 w-6 shrink-0 {{ request()->routeIs('outgoingemails.*') ? 'text-gray-700' : 'text-gray-500 group-hover:text-gray-900' }}" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                    ...
                    @if($user && ($user->hasPermission('manage_roles') || $user->hasPermission('manage_permissions') || $user->hasPermission('view_clients') || $user->hasPermission('edit_my_client_settings') || $user->hasPermission('view_orders')))
                        <li>
                            <button @click="adminSubmenu = !adminSubmenu" class="group flex w-full items-center gap-x-3 rounded-md p-2 text-sm font-semibold {{ (request()->routeIs('roles.*') || request()->routeIs('permissions.*') || request()->routeIs('clients.*') || request()->routeIs('users.*') || request()->routeIs('logos.*') || request()->routeIs('clients.my-settings') || request()->routeIs('orders.*')) ? 'bg-gray-100 text-gray-900' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900' }} focus:outline-none">
                                <!-- SVG Icon -->
            ...
            <li class="-mx-6 mt-auto border-t border-gray-200">
                <div class="flex items-center gap-x-2 px-6 py-3">
                    <a href="{{ route('profile.edit') }}" class="flex items-center gap-x-4 flex-1 rounded-md hover:bg-gray-100 px-2 py-2">
                        <div class="h-8 w-8 bg-blue-900 rounded-lg flex items-center justify-center">
