<x-layout>
        ...
        <div class="md:hidden text-center mb-4">
            ...
            <p class="text-gray-600 mt-1">
                @if ($invoicepositioncontent->positiontext == 0)
                    {{ $invoicepositioncontent->designation }}
                @else
                    Textposition
                @endif
            </p>
    ...
    @if ($invoicepositioncontent->positiontext == 0)
        <div class="bg-white/60 backdrop-blur-lg rounded-xl p-6 border border-white/20 shadow-lg">
            <div class="mb-6">
                <h2 class="text-lg font-semibold text-gray-900 flex items-center">
                    <div class="w-8 h-8 bg-blue-500 rounded-lg flex items-center justify-center text-white font-bold text-xs mr-3">
                        {{ $invoicepositioncontent->sequence }}
                    </div>
                    Position bearbeiten
                </h2>
            ...
            <form method="POST" action="{{ route('invoiceposition.update', ['invoiceposition' => $invoicepositioncontent->id]) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <x-input type="text" name="designation" id="designation" value="{{ old('designation', $invoicepositioncontent->designation) }}" placeholder="Beschreibung eingeben" />
                </div>
