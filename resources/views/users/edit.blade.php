<x-layout>
    <div class="container">
        <div class="row">
            ...
            <div class="col">
                <a href="{{ route('users.index') }}" class="btn btn-transparent float-right">Zurück</a>
            </div>
        </div>
        <form action="includes/user_update.inc.php" method="POST">
            <div class = "row">
                <div class = "col">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="rightsId" name="rightsId" required>
                            @foreach($rights as $right)
                                <option value="{{ $right->id }}" {{ $right->id == old('right_id', $user->role_id) ? 'selected' : '' }}>
                                    {{ $right->name }}
                                </option>
                            @endforeach
                        </select>
