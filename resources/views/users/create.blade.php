<x-layout>
    ...
    <div class="grid grid-cols-1 gap-x-8 border-b border-gray-900/10 pb-12 md:grid-cols-7 sm:grid-cols-1">
        ...
        <!-- Formular -->
        <form action="{{ route('users.create') }}" method="GET" class="sm:col-span-1 md:col-span-5 ">
            <div class="grid md:grid-cols-4 sm:grid-cols-1 pb-4 gap-x-6">
                ...
                <!-- Nachname -->
                <div class="sm:col-span-1">
                    <x-input name="lastname" type="text" placeholder="Nachname" label="Nachname" value="" />
                </div>
            </div>
            ...
            <div class="grid md:grid-cols-4 sm:grid-cols-1 pb-4 gap-x-6 border-b">
                <!-- Loginname -->
                <div class="sm:col-span-1">
                    <x-input name="login" type="text" placeholder="Loginname" label="Loginname" value="" />
                </div>

                <!-- Klient -->
                <div class="sm:col-span-1">
                    <x-dropdown_body name="client_id" id="client_id" :options="$clients->pluck('clientname', 'id')" :selected="1" label="Klient" placeholder="Bitte auswählen" class="w-full appearance-none rounded-md bg-white py-1.5 pl-3 pr-8 text-base text-gray-900 outline outline-1 -outline-offset-1 outline-gray-300 focus:outline focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm" />
                </div>
            </div>
            <div class="grid md:grid-cols-4 sm:grid-cols-1 pb-4 gap-x-6 border-b">
                <!-- Email -->
                <div class="sm:col-span-1">
                    <x-input name="lemailogin" type="text" placeholder="Email" label="E-Mail" value="" />
                </div>

                <!-- Aktiv -->
                <div class="sm:col-span-1">
                    <x-dropdown_body name="isactive" id="isactive" :options="['1' => 'Ja', '0' => 'Nein']" :selected="1" label="Aktiv" placeholder="Bitte auswählen" class="w-full appearance-none rounded-md bg-white py-1.5 pl-3 pr-8 text-base text-gray-900 outline outline-1 -outline-offset-1 outline-gray-300 focus:outline focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm" />
                </div>

                <!-- Rolle -->
                <div class="sm:col-span-1">
                    <x-dropdown_body name="role_id" id="role_id" :options="$roles->pluck('name', 'id')" :selected="1" label="Rolle" placeholder="Bitte auswählen" />
                </div>
