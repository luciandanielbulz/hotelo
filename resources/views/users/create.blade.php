    ...
    <div class="grid grid-cols-1 gap-x-8 border-b border-gray-900/10 pb-12 md:grid-cols-7 sm:grid-cols-1">
        ...
        <form action="{{ route('users.store') }}" method="POST" class="sm:col-span-1 md:col-span-5 ">
            ...
            <div class="grid md:grid-cols-4 sm:grid-cols-1 pb-4 gap-x-6 border-b">
                ...
                <!-- Rolle -->
                <div class="sm:col-span-1">
                    <x-dropdown_body name="role_id" id="role_id" :options="$roles->pluck('name', 'id')" :selected="old('role_id', 1)" label="Rolle" placeholder="Bitte auswählen" class="w-full appearance-none rounded-md bg-white py-1.5 pl-3 pr-8 text-base text-gray-900 outline outline-1 -outline-offset-1 outline-gray-300 focus:outline focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm" />
                </div>
